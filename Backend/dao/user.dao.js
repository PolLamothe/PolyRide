...
const userDAO = {
	...
	async updateProfile(email, newUsage, newCalendarLink,adresse,position){
		const db = getDb();
		db.collection("users").updateOne({ email: email }, { $set: { calendarLink: newCalendarLink, usage : newUsage,adresse : adresse, position : position} })
	}
};
